...
$lov_countries_view = NULL; // Initialize page object first

class clov_countries_view extends clov_countries {
	...
	// Page object name
	var $PageObjName = 'lov_countries_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "lov_countries_list.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "lov_countries_list.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "lov_countries_list.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_lov_countries\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_lov_countries',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.flov_countriesview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($lov_countries_view)) $lov_countries_view = new clov_countries_view();

// Page init
$lov_countries_view->Page_Init();

// Page main
$lov_countries_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$lov_countries_view->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($lov_countries->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "view";
var CurrentForm = flov_countriesview = new ew_Form("flov_countriesview", "view");

// Form_CustomValidate event
flov_countriesview.Form_CustomValidate = 
...
flov_countriesview.ValidateRequired = true;
<?php } else { ?>
flov_countriesview.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $lov_countries_view->ExportOptions->Render("body") ?>
<?php
	foreach ($lov_countries_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($lov_countries->Export == "") { ?>
...
<?php } ?>
<?php $lov_countries_view->ShowPageHeader(); ?>
<?php
$lov_countries_view->ShowMessage();
?>
<form name="flov_countriesview" id="flov_countriesview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($lov_countries_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $lov_countries_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="lov_countries">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($lov_countries->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_lov_countries_id"><?php echo $lov_countries->id->FldCaption() ?></span></td>
		<td data-name="id"<?php echo $lov_countries->id->CellAttributes() ?>>
<span id="el_lov_countries_id">
<span<?php echo $lov_countries->id->ViewAttributes() ?>>
<?php echo $lov_countries->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($lov_countries->countryCode->Visible) { // countryCode ?>
	<tr id="r_countryCode">
		<td><span id="elh_lov_countries_countryCode"><?php echo $lov_countries->countryCode->FldCaption() ?></span></td>
		<td data-name="countryCode"<?php echo $lov_countries->countryCode->CellAttributes() ?>>
<span id="el_lov_countries_countryCode">
<span<?php echo $lov_countries->countryCode->ViewAttributes() ?>>
<?php echo $lov_countries->countryCode->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($lov_countries->countryName->Visible) { // countryName ?>
	<tr id="r_countryName">
		<td><span id="elh_lov_countries_countryName"><?php echo $lov_countries->countryName->FldCaption() ?></span></td>
		<td data-name="countryName"<?php echo $lov_countries->countryName->CellAttributes() ?>>
<span id="el_lov_countries_countryName">
<span<?php echo $lov_countries->countryName->ViewAttributes() ?>>
<?php echo $lov_countries->countryName->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($lov_countries->currencyCode->Visible) { // currencyCode ?>
	<tr id="r_currencyCode">
		<td><span id="elh_lov_countries_currencyCode"><?php echo $lov_countries->currencyCode->FldCaption() ?></span></td>
		<td data-name="currencyCode"<?php echo $lov_countries->currencyCode->CellAttributes() ?>>
<span id="el_lov_countries_currencyCode">
<span<?php echo $lov_countries->currencyCode->ViewAttributes() ?>>
<?php echo $lov_countries->currencyCode->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($lov_countries->isoNumeric->Visible) { // isoNumeric ?>
	<tr id="r_isoNumeric">
		<td><span id="elh_lov_countries_isoNumeric"><?php echo $lov_countries->isoNumeric->FldCaption() ?></span></td>
		<td data-name="isoNumeric"<?php echo $lov_countries->isoNumeric->CellAttributes() ?>>
<span id="el_lov_countries_isoNumeric">
<span<?php echo $lov_countries->isoNumeric->ViewAttributes() ?>>
<?php echo $lov_countries->isoNumeric->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($lov_countries->isoAlpha3->Visible) { // isoAlpha3 ?>
	<tr id="r_isoAlpha3">
		<td><span id="elh_lov_countries_isoAlpha3"><?php echo $lov_countries->isoAlpha3->FldCaption() ?></span></td>
		<td data-name="isoAlpha3"<?php echo $lov_countries->isoAlpha3->CellAttributes() ?>>
<span id="el_lov_countries_isoAlpha3">
<span<?php echo $lov_countries->isoAlpha3->ViewAttributes() ?>>
<?php echo $lov_countries->isoAlpha3->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($lov_countries->geonameId->Visible) { // geonameId ?>
	<tr id="r_geonameId">
		<td><span id="elh_lov_countries_geonameId"><?php echo $lov_countries->geonameId->FldCaption() ?></span></td>
		<td data-name="geonameId"<?php echo $lov_countries->geonameId->CellAttributes() ?>>
<span id="el_lov_countries_geonameId">
<span<?php echo $lov_countries->geonameId->ViewAttributes() ?>>
<?php echo $lov_countries->geonameId->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($lov_countries->Export == "") { ?>
<script type="text/javascript">
flov_countriesview.Init();
</script>
<?php } ?>
<?php
$lov_countries_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($lov_countries->Export == "") { ?>
...
<?php
$lov_countries_view->Page_Terminate();
?>
