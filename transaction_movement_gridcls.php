...
$transaction_Movement_grid = NULL; // Initialize page object first

class ctransaction_Movement_grid extends ctransaction_Movement {
	...
	// Page object name
	var $PageObjName = 'transaction_Movement_grid';

	// Grid form hidden field names
	var $FormName = 'ftransaction_Movementgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["transaction_Movement"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->tran_ID->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_tr_type") && $objForm->HasValue("o_tr_type") && $this->tr_type->CurrentValue <> $this->tr_type->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_tran_Detail") && $objForm->HasValue("o_tran_Detail") && $this->tran_Detail->CurrentValue <> $this->tran_Detail->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_s_ID") && $objForm->HasValue("o_s_ID") && $this->s_ID->CurrentValue <> $this->s_ID->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_tran_show") && $objForm->HasValue("o_tran_show") && $this->tran_show->CurrentValue <> $this->tran_show->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->tran_ID->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->tran_Created->OldValue = $this->tran_Created->CurrentValue;
		...
		$this->tran_ID->CurrentValue = NULL;
		$this->tran_ID->OldValue = $this->tran_ID->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->tran_Created->setOldValue($objForm->GetValue("o_tran_Created"));
		if (!$this->u_ID->FldIsDetailKey) {
			$this->u_ID->setFormValue($objForm->GetValue("x_u_ID"));
		}
		$this->u_ID->setOldValue($objForm->GetValue("o_u_ID"));
		if (!$this->tr_type->FldIsDetailKey) {
			$this->tr_type->setFormValue($objForm->GetValue("x_tr_type"));
		}
		$this->tr_type->setOldValue($objForm->GetValue("o_tr_type"));
		if (!$this->tran_Detail->FldIsDetailKey) {
			$this->tran_Detail->setFormValue($objForm->GetValue("x_tran_Detail"));
		}
		$this->tran_Detail->setOldValue($objForm->GetValue("o_tran_Detail"));
		if (!$this->s_ID->FldIsDetailKey) {
			$this->s_ID->setFormValue($objForm->GetValue("x_s_ID"));
		}
		$this->s_ID->setOldValue($objForm->GetValue("o_s_ID"));
		if (!$this->tran_ID->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
			$this->tran_ID->setFormValue($objForm->GetValue("x_tran_ID"));
		if (!$this->tran_show->FldIsDetailKey) {
			$this->tran_show->setFormValue($objForm->GetValue("x_tran_show"));
		}
		$this->tran_show->setOldValue($objForm->GetValue("o_tran_show"));
	}
	...
	function RestoreFormValues() {
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
			$this->tran_ID->CurrentValue = $this->tran_ID->FormValue;
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->tran_ID->CurrentValue = strval($arKeys[0]); // tran_ID
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// tr_type
			$this->tr_type->SetDbValueDef($rsnew, $this->tr_type->CurrentValue, 0, $this->tr_type->ReadOnly);

			// tran_Detail
			$this->tran_Detail->SetDbValueDef($rsnew, $this->tran_Detail->CurrentValue, "", $this->tran_Detail->ReadOnly);

			// s_ID
			$this->s_ID->SetDbValueDef($rsnew, $this->s_ID->CurrentValue, NULL, $this->s_ID->ReadOnly);

			// tran_show
			$this->tran_show->SetDbValueDef($rsnew, $this->tran_show->CurrentValue, 0, $this->tran_show->ReadOnly);
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->pr_ID->getSessionValue() <> "") {
			$rsnew['pr_ID'] = $this->pr_ID->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "main_Product") {
			...
			if ($GLOBALS["main_Product"]->EventCancelled) $this->EventCancelled = TRUE;
		}
