...
$main_Brand_delete = NULL; // Initialize page object first

class cmain_Brand_delete extends cmain_Brand {
	...
	// Page object name
	var $PageObjName = 'main_Brand_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("main_brand_list.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in main_Brand class, main_Brandinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("main_brand_list.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($main_Brand_delete)) $main_Brand_delete = new cmain_Brand_delete();

// Page init
$main_Brand_delete->Page_Init();

// Page main
$main_Brand_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$main_Brand_delete->Page_Render();
?>
...
var CurrentForm = fmain_Branddelete = new ew_Form("fmain_Branddelete", "delete");

// Form_CustomValidate event
fmain_Branddelete.Form_CustomValidate = 
...
fmain_Branddelete.ValidateRequired = true;
<?php } else { ?>
fmain_Branddelete.ValidateRequired = false; 
<?php } ?>
...
<?php $main_Brand_delete->ShowPageHeader(); ?>
<?php
$main_Brand_delete->ShowMessage();
?>
<form name="fmain_Branddelete" id="fmain_Branddelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($main_Brand_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $main_Brand_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="main_Brand">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($main_Brand_delete->RecKeys as $key) { ?>
...
<?php if ($main_Brand->logo->Visible) { // logo ?>
		<th><span id="elh_main_Brand_logo" class="main_Brand_logo"><?php echo $main_Brand->logo->FldCaption() ?></span></th>
<?php } ?>
<?php if ($main_Brand->b_Name->Visible) { // b_Name ?>
		<th><span id="elh_main_Brand_b_Name" class="main_Brand_b_Name"><?php echo $main_Brand->b_Name->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$main_Brand_delete->RecCnt = 0;
$i = 0;
while (!$main_Brand_delete->Recordset->EOF) {
	$main_Brand_delete->RecCnt++;
	$main_Brand_delete->RowCnt++;

	// Set row properties
	$main_Brand->ResetAttrs();
	$main_Brand->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$main_Brand_delete->LoadRowValues($main_Brand_delete->Recordset);

	// Render row
	$main_Brand_delete->RenderRow();
?>
	<tr<?php echo $main_Brand->RowAttributes() ?>>
<?php if ($main_Brand->logo->Visible) { // logo ?>
		<td<?php echo $main_Brand->logo->CellAttributes() ?>>
<span id="el<?php echo $main_Brand_delete->RowCnt ?>_main_Brand_logo" class="main_Brand_logo">
<span>
...
<span id="el<?php echo $main_Brand_delete->RowCnt ?>_main_Brand_b_Name" class="main_Brand_b_Name">
...
<?php
	$main_Brand_delete->Recordset->MoveNext();
}
$main_Brand_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $main_Brand_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fmain_Branddelete.Init();
</script>
<?php
$main_Brand_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$main_Brand_delete->Page_Terminate();
?>
