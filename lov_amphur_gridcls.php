...
$lov_amphur_grid = NULL; // Initialize page object first

class clov_amphur_grid extends clov_amphur {
	...
	// Page object name
	var $PageObjName = 'lov_amphur_grid';

	// Grid form hidden field names
	var $FormName = 'flov_amphurgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["lov_amphur"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->AMPHUR_ID->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_AMPHUR_CODE") && $objForm->HasValue("o_AMPHUR_CODE") && $this->AMPHUR_CODE->CurrentValue <> $this->AMPHUR_CODE->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_PROVINCE_ID") && $objForm->HasValue("o_PROVINCE_ID") && $this->PROVINCE_ID->CurrentValue <> $this->PROVINCE_ID->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_AMPHUR_NAME") && $objForm->HasValue("o_AMPHUR_NAME") && $this->AMPHUR_NAME->CurrentValue <> $this->AMPHUR_NAME->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_AMPER_NAME_EN") && $objForm->HasValue("o_AMPER_NAME_EN") && $this->AMPER_NAME_EN->CurrentValue <> $this->AMPER_NAME_EN->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->AMPHUR_ID->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->AMPHUR_ID->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->AMPHUR_CODE->setOldValue($objForm->GetValue("o_AMPHUR_CODE"));
		...
		$this->AMPHUR_NAME->setOldValue($objForm->GetValue("o_AMPHUR_NAME"));
		if (!$this->AMPER_NAME_EN->FldIsDetailKey) {
			$this->AMPER_NAME_EN->setFormValue($objForm->GetValue("x_AMPER_NAME_EN"));
		}
		$this->AMPER_NAME_EN->setOldValue($objForm->GetValue("o_AMPER_NAME_EN"));
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->AMPHUR_ID->CurrentValue = strval($arKeys[0]); // AMPHUR_ID
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "lov_province") {
			...
			if ($GLOBALS["lov_province"]->EventCancelled) $this->EventCancelled = TRUE;
		}
