...
$main_PartNum_preview = NULL; // Initialize page object first

class cmain_PartNum_preview extends cmain_PartNum {
	...
	// Page object name
	var $PageObjName = 'main_PartNum_preview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'main_PartNum');
	...
	function MasterKeyUrl() {
		$mastertblvar = @$_GET["t"];
		$url = "";
		if ($mastertblvar == "main_Brand") {
			$url = "" . EW_TABLE_SHOW_MASTER . "=main_Brand&fk_b_ID=" . urlencode(strval($this->b_ID->QueryStringValue)) . "";
		}
		if ($mastertblvar == "main_Vendor") {
			$url = "" . EW_TABLE_SHOW_MASTER . "=main_Vendor&fk_v_ID=" . urlencode(strval($this->v_ID->QueryStringValue)) . "";
		}
		return $url;
	}
		...
		if ($mastertblvar == "main_Brand") {
			...
			if ($x !== FALSE) {
				...
				$val = $this->UnquoteValue($val, "DB");
 				$this->b_ID->setQueryStringValue($val);
			}
		}
		if ($mastertblvar == "main_Vendor") {
			$find = "`v_ID`="; 
			...
			if ($x !== FALSE) {
				...
				$val = substr($f, $x);
				$val = $this->UnquoteValue($val, "DB");
 				$this->v_ID->setQueryStringValue($val);
			}
		}
	}
...
// Create page object
if (!isset($main_PartNum_preview)) $main_PartNum_preview = new cmain_PartNum_preview();

// Page init
$main_PartNum_preview->Page_Init();

// Page main
$main_PartNum_preview->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$main_PartNum_preview->Page_Render();
?>
<?php $main_PartNum_preview->ShowPageHeader(); ?>
<?php if ($main_PartNum_preview->TotalRecs > 0) { ?>
<div class="ewGrid">
...
// Render list options
$main_PartNum_preview->RenderListOptions();

// Render list options (header, left)
$main_PartNum_preview->ListOptions->Render("header", "left");
?>
<?php if ($main_PartNum->pn_Barcode->Visible) { // pn_Barcode ?>
			<th><?php echo $main_PartNum->pn_Barcode->FldCaption() ?></th>
<?php } ?>
<?php if ($main_PartNum->v_ID->Visible) { // v_ID ?>
			<th><?php echo $main_PartNum->v_ID->FldCaption() ?></th>
<?php } ?>
<?php if ($main_PartNum->b_ID->Visible) { // b_ID ?>
			<th><?php echo $main_PartNum->b_ID->FldCaption() ?></th>
<?php } ?>
<?php if ($main_PartNum->pn_ProductName->Visible) { // pn_ProductName ?>
			<th><?php echo $main_PartNum->pn_ProductName->FldCaption() ?></th>
<?php } ?>
<?php

// Render list options (header, right)
$main_PartNum_preview->ListOptions->Render("header", "right");
?>
		</tr>
	</thead>
	<tbody><!-- Table body -->
<?php
$main_PartNum_preview->RecCount = 0;
$main_PartNum_preview->RowCnt = 0;
while ($main_PartNum_preview->Recordset && !$main_PartNum_preview->Recordset->EOF) {

	// Init row class and style
	$main_PartNum_preview->RecCount++;
	$main_PartNum_preview->RowCnt++;
	$main_PartNum_preview->CssStyle = "";
	$main_PartNum_preview->LoadListRowValues($main_PartNum_preview->Recordset);

	// Render row
	$main_PartNum_preview->RowType = EW_ROWTYPE_PREVIEW; // Preview record
	$main_PartNum_preview->RenderListRow();

	// Render list options
	$main_PartNum_preview->RenderListOptions();
?>
	<tr<?php echo $main_PartNum_preview->RowAttributes() ?>>
<?php

// Render list options (body, left)
$main_PartNum_preview->ListOptions->Render("body", "left", $main_PartNum_preview->RowCnt);
?>
<?php if ($main_PartNum->pn_Barcode->Visible) { // pn_Barcode ?>
		<!-- pn_Barcode -->
...
$main_PartNum_preview->ListOptions->Render("body", "right", $main_PartNum_preview->RowCnt);
?>
	</tr>
<?php
	$main_PartNum_preview->Recordset->MoveNext();
}
?>
	</tbody>
</table>
...
<?php if ($main_PartNum_preview->TotalRecs > 0) { ?>
<div class="ewDetailCount"><?php echo $main_PartNum_preview->TotalRecs ?>&nbsp;<?php echo $Language->Phrase("Record") ?></div>
<?php } else { ?>
<div class="ewDetailCount"><?php echo $Language->Phrase("NoRecord") ?></div>
<?php } ?>
<div class="ewPreviewOtherOptions">
<?php
	foreach ($main_PartNum_preview->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
...
<?php
$main_PartNum_preview->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php
if ($main_PartNum_preview->Recordset)
	$main_PartNum_preview->Recordset->Close();
...
?>
<?php
$main_PartNum_preview->Page_Terminate();
?>
