...
function ew_AutoLoad($class) {
	...
	if ($class == "GD") {
		...
		$file = str_replace("%cls%", substr($class, 1), "%cls%_info.php");
...
function CurrentProjectID() {
	...
	return "{B1D96CD0-2849-4DC1-8F87-20EC273F9356}";
}
...
class cAdvancedSecurity {
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE) {
		...
		if (!$ValidateUser) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ew_ComparePassword($rs->fields('u_Passwd'), $pwd, $encrypted);
					...
					if ($ValidateUser) {
						...
						$this->setCurrentUserName($rs->fields('u_LoginName')); // Load user name
						if (is_null($rs->fields('u_level'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('u_level'))); // Load User Level
						}
						$this->SetUpUserLevel();
	...
	function SetUpUserLevelEx() {
		...
		if (count($this->UserLevelPriv) == 0 && $this->IsAdmin() && $Page != NULL && @$_SESSION[EW_SESSION_USER_LEVEL_MSG] == "") {
			...
			$Page->Page_Terminate("userlevels_list.php");
		}
		return TRUE;
	}
	...
	function CurrentUserEmail() {
		return $this->CurrentUserInfo("u_Email");
	}
