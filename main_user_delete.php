...
$main_User_delete = NULL; // Initialize page object first

class cmain_User_delete extends cmain_User {
	...
	// Page object name
	var $PageObjName = 'main_User_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("main_user_list.php"); // Prevent SQL injection, return to list
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("main_user_list.php"); // Return to list
			}
		}
	}
	...
	function LoadRowValues(&$rs) {
		...
		$this->u_UUID->setDbValue($rs->fields('u_UUID'));
	...
	function LoadDbValues(&$rs) {
		...
		$this->u_UUID->DbValue = $row['u_UUID'];
...
// Create page object
if (!isset($main_User_delete)) $main_User_delete = new cmain_User_delete();

// Page init
$main_User_delete->Page_Init();

// Page main
$main_User_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$main_User_delete->Page_Render();
?>
...
var CurrentForm = fmain_Userdelete = new ew_Form("fmain_Userdelete", "delete");

// Form_CustomValidate event
fmain_Userdelete.Form_CustomValidate = 
...
fmain_Userdelete.ValidateRequired = true;
<?php } else { ?>
fmain_Userdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmain_Userdelete.Lists["x_u_Status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmain_Userdelete.Lists["x_u_Status"].Options = <?php echo json_encode($main_User->u_Status->Options()) ?>;
fmain_Userdelete.Lists["x_u_level"] = {"LinkField":"x_userlevelid","Ajax":true,"AutoFill":false,"DisplayFields":["x_userlevelname","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $main_User_delete->ShowPageHeader(); ?>
<?php
$main_User_delete->ShowMessage();
?>
<form name="fmain_Userdelete" id="fmain_Userdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($main_User_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $main_User_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="main_User">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($main_User_delete->RecKeys as $key) { ?>
...
<?php if ($main_User->u_LoginName->Visible) { // u_LoginName ?>
		<th><span id="elh_main_User_u_LoginName" class="main_User_u_LoginName"><?php echo $main_User->u_LoginName->FldCaption() ?></span></th>
<?php } ?>
<?php if ($main_User->u_Email->Visible) { // u_Email ?>
		<th><span id="elh_main_User_u_Email" class="main_User_u_Email"><?php echo $main_User->u_Email->FldCaption() ?></span></th>
<?php } ?>
<?php if ($main_User->u_BillName->Visible) { // u_BillName ?>
		<th><span id="elh_main_User_u_BillName" class="main_User_u_BillName"><?php echo $main_User->u_BillName->FldCaption() ?></span></th>
<?php } ?>
<?php if ($main_User->u_Mobile->Visible) { // u_Mobile ?>
		<th><span id="elh_main_User_u_Mobile" class="main_User_u_Mobile"><?php echo $main_User->u_Mobile->FldCaption() ?></span></th>
<?php } ?>
<?php if ($main_User->u_Status->Visible) { // u_Status ?>
		<th><span id="elh_main_User_u_Status" class="main_User_u_Status"><?php echo $main_User->u_Status->FldCaption() ?></span></th>
<?php } ?>
<?php if ($main_User->u_level->Visible) { // u_level ?>
		<th><span id="elh_main_User_u_level" class="main_User_u_level"><?php echo $main_User->u_level->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$main_User_delete->RecCnt = 0;
$i = 0;
while (!$main_User_delete->Recordset->EOF) {
	$main_User_delete->RecCnt++;
	$main_User_delete->RowCnt++;

	// Set row properties
	$main_User->ResetAttrs();
	$main_User->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$main_User_delete->LoadRowValues($main_User_delete->Recordset);

	// Render row
	$main_User_delete->RenderRow();
?>
	<tr<?php echo $main_User->RowAttributes() ?>>
<?php if ($main_User->u_LoginName->Visible) { // u_LoginName ?>
		<td<?php echo $main_User->u_LoginName->CellAttributes() ?>>
<span id="el<?php echo $main_User_delete->RowCnt ?>_main_User_u_LoginName" class="main_User_u_LoginName">
<span<?php echo $main_User->u_LoginName->ViewAttributes() ?>>
<?php echo $main_User->u_LoginName->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
<?php if ($main_User->u_Email->Visible) { // u_Email ?>
		<td<?php echo $main_User->u_Email->CellAttributes() ?>>
<span id="el<?php echo $main_User_delete->RowCnt ?>_main_User_u_Email" class="main_User_u_Email">
<span<?php echo $main_User->u_Email->ViewAttributes() ?>>
<?php echo $main_User->u_Email->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
<?php if ($main_User->u_BillName->Visible) { // u_BillName ?>
		<td<?php echo $main_User->u_BillName->CellAttributes() ?>>
<span id="el<?php echo $main_User_delete->RowCnt ?>_main_User_u_BillName" class="main_User_u_BillName">
...
<?php } ?>
<?php if ($main_User->u_Mobile->Visible) { // u_Mobile ?>
		<td<?php echo $main_User->u_Mobile->CellAttributes() ?>>
<span id="el<?php echo $main_User_delete->RowCnt ?>_main_User_u_Mobile" class="main_User_u_Mobile">
<span<?php echo $main_User->u_Mobile->ViewAttributes() ?>>
<?php echo $main_User->u_Mobile->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
<?php if ($main_User->u_Status->Visible) { // u_Status ?>
		<td<?php echo $main_User->u_Status->CellAttributes() ?>>
<span id="el<?php echo $main_User_delete->RowCnt ?>_main_User_u_Status" class="main_User_u_Status">
...
<?php } ?>
<?php if ($main_User->u_level->Visible) { // u_level ?>
		<td<?php echo $main_User->u_level->CellAttributes() ?>>
<span id="el<?php echo $main_User_delete->RowCnt ?>_main_User_u_level" class="main_User_u_level">
<span<?php echo $main_User->u_level->ViewAttributes() ?>>
<?php echo $main_User->u_level->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
	</tr>
<?php
	$main_User_delete->Recordset->MoveNext();
}
$main_User_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $main_User_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fmain_Userdelete.Init();
</script>
<?php
$main_User_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$main_User_delete->Page_Terminate();
?>
