...
$main_Product_grid = NULL; // Initialize page object first

class cmain_Product_grid extends cmain_Product {
	...
	// Page object name
	var $PageObjName = 'main_Product_grid';

	// Grid form hidden field names
	var $FormName = 'fmain_Productgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["main_Product"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->pr_ID->CurrentValue = strval($arKeys[0]); // pr_ID
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
