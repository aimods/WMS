...
//------------------------------------------------------------------------------
//
// PHP Connection Script for PHPMaker 12
...
<p>(C) 2015 e.World Technology Ltd.</p>
</body>
</html>
<?php
//------------------------------------------------------------------------------
...
class MetaMySQL {
	...
	function __construct() {
		...
		if (is_numeric($port)) {
			$this->Link = @mysql_connect($host . ":" . intval($port), $uid, $pwd) or die(mysql_error()); // P12
		} else {
			$this->Link = @mysql_connect($host, $uid, $pwd) or die(mysql_error()); // P12
		}
...
//		mysql_select_db($dbname) or die(mysql_error());
		if ($sql == "") { // table
			$result1 = mysql_query("SHOW COLUMNS FROM " . $quotechr . str_replace($quotechr, $quotechr . $quotechr, $tablename) . $quotechr) or die(mysql_error());
			$result2 = mysql_query("SELECT * FROM " . $quotechr . str_replace($quotechr, $quotechr . $quotechr, $tablename) . $quotechr . " LIMIT 1") or die(mysql_error());
			if ($result1) {
				$res = array();
...
//------------------------------------------------------------------------------
//
// Meta data for MySQL (MySQLi) // P12
//
//------------------------------------------------------------------------------
