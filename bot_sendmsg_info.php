...
//
// Table class for bot_SendMSG
//
class cbot_SendMSG extends cTable {
	var $sm_ID;
	var $u_ID;
	var $u_sendTo;
	var $u_TextType;
	...
	function __construct() {
		...
		$this->TableVar = 'bot_SendMSG';
		$this->TableName = 'bot_SendMSG';
		...
		// sm_ID
		$this->sm_ID = new cField('bot_SendMSG', 'bot_SendMSG', 'x_sm_ID', 'sm_ID', '`sm_ID`', '`sm_ID`', 3, -1, FALSE, '`sm_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->sm_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['sm_ID'] = &$this->sm_ID;

		// u_ID
		$this->u_ID = new cField('bot_SendMSG', 'bot_SendMSG', 'x_u_ID', 'u_ID', '`u_ID`', '`u_ID`', 3, -1, FALSE, '`u_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		$this->u_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['u_ID'] = &$this->u_ID;

		// u_sendTo
		$this->u_sendTo = new cField('bot_SendMSG', 'bot_SendMSG', 'x_u_sendTo', 'u_sendTo', '`u_sendTo`', '`u_sendTo`', 200, -1, FALSE, '`u_sendTo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['u_sendTo'] = &$this->u_sendTo;

		// u_TextType
		$this->u_TextType = new cField('bot_SendMSG', 'bot_SendMSG', 'x_u_TextType', 'u_TextType', '`u_TextType`', '`u_TextType`', 3, -1, FALSE, '`u_TextType`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// u_sendText
		$this->u_sendText = new cField('bot_SendMSG', 'bot_SendMSG', 'x_u_sendText', 'u_sendText', '`u_sendText`', '`u_sendText`', 201, -1, FALSE, '`u_sendText`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['u_sendText'] = &$this->u_sendText;

		// u_sendSchedule
		$this->u_sendSchedule = new cField('bot_SendMSG', 'bot_SendMSG', 'x_u_sendSchedule', 'u_sendSchedule', '`u_sendSchedule`', 'DATE_FORMAT(`u_sendSchedule`, \'%d/%m/%Y\')', 135, 7, FALSE, '`u_sendSchedule`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// u_Status
		$this->u_Status = new cField('bot_SendMSG', 'bot_SendMSG', 'x_u_Status', 'u_Status', '`u_Status`', '`u_Status`', 3, -1, FALSE, '`u_Status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->u_Status->OptionCount = 4;
		$this->fields['u_Status'] = &$this->u_Status;

		// u_Create
		$this->u_Create = new cField('bot_SendMSG', 'bot_SendMSG', 'x_u_Create', 'u_Create', '`u_Create`', 'DATE_FORMAT(`u_Create`, \'%d/%m/%Y\')', 135, 7, FALSE, '`u_Create`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->u_Create->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['u_Create'] = &$this->u_Create;

		// u_Sent
		$this->u_Sent = new cField('bot_SendMSG', 'bot_SendMSG', 'x_u_Sent', 'u_Sent', '`u_Sent`', 'DATE_FORMAT(`u_Sent`, \'%d/%m/%Y\')', 135, 7, FALSE, '`u_Sent`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`u_Create` DESC";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('sm_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('sm_ID', $this->DBID) . '=' . ew_QuotedValue($rs['sm_ID'], $this->sm_ID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->sm_ID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@sm_ID@", ew_AdjustSql($this->sm_ID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("bot_sendmsg_view.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("bot_sendmsg_view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "bot_sendmsg_add.php?" . $this->UrlParm($parm);
		else
			$url = "bot_sendmsg_add.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("bot_sendmsg_edit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("bot_sendmsg_add.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("bot_sendmsg_delete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "sm_ID:" . ew_VarToJson($this->sm_ID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->sm_ID->CurrentValue)) {
			$sUrl .= "sm_ID=" . urlencode($this->sm_ID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["sm_ID"]))
				$arKeys[] = ew_StripSlashes($_POST["sm_ID"]);
			elseif (isset($_GET["sm_ID"]))
				$arKeys[] = ew_StripSlashes($_GET["sm_ID"]);
			else
