...
$main_Vendor_edit = NULL; // Initialize page object first

class cmain_Vendor_edit extends cmain_Vendor {
	...
	// Page object name
	var $PageObjName = 'main_Vendor_edit';
	...
	function Page_Init() {
		...
		$this->v_ID->Visible = !$this->IsAdd() && !$this->IsCopy() && !$this->IsGridAdd();
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->v_ID->CurrentValue == "")
			$this->Page_Terminate("main_vendor_list.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($this->v_TAX->CurrentValue <> "") { // Check field with unique index
			$sFilterChk = "(`v_TAX` = '" . ew_AdjustSql($this->v_TAX->CurrentValue, $this->DBID) . "')";
		...
		if ($rs->EOF) {
			...
			// v_Name
			$this->v_Name->SetDbValueDef($rsnew, $this->v_Name->CurrentValue, "", $this->v_Name->ReadOnly);

			// v_TAX
			$this->v_TAX->SetDbValueDef($rsnew, $this->v_TAX->CurrentValue, NULL, $this->v_TAX->ReadOnly);

			// v_Country
			$this->v_Country->SetDbValueDef($rsnew, $this->v_Country->CurrentValue, 0, $this->v_Country->ReadOnly);

			// v_BillingAddress
			$this->v_BillingAddress->SetDbValueDef($rsnew, $this->v_BillingAddress->CurrentValue, NULL, $this->v_BillingAddress->ReadOnly);

			// v_Contact
			$this->v_Contact->SetDbValueDef($rsnew, $this->v_Contact->CurrentValue, NULL, $this->v_Contact->ReadOnly);

			// v_Note
			$this->v_Note->SetDbValueDef($rsnew, $this->v_Note->CurrentValue, NULL, $this->v_Note->ReadOnly);
...
// Create page object
if (!isset($main_Vendor_edit)) $main_Vendor_edit = new cmain_Vendor_edit();

// Page init
$main_Vendor_edit->Page_Init();

// Page main
$main_Vendor_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$main_Vendor_edit->Page_Render();
?>
...
var CurrentForm = fmain_Vendoredit = new ew_Form("fmain_Vendoredit", "edit");

// Validate form
fmain_Vendoredit.Validate = function() {
...
fmain_Vendoredit.Form_CustomValidate = 
...
fmain_Vendoredit.ValidateRequired = true;
<?php } else { ?>
fmain_Vendoredit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmain_Vendoredit.Lists["x_v_Country"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_countryName","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmain_Vendoredit.Lists["x_v_Contact"] = {"LinkField":"x_u_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_u_BillName","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $main_Vendor_edit->ShowPageHeader(); ?>
<?php
$main_Vendor_edit->ShowMessage();
?>
<form name="fmain_Vendoredit" id="fmain_Vendoredit" class="<?php echo $main_Vendor_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($main_Vendor_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $main_Vendor_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="main_Vendor">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<div>
<?php if ($main_Vendor->v_ID->Visible) { // v_ID ?>
	<div id="r_v_ID" class="form-group">
		<label id="elh_main_Vendor_v_ID" class="col-sm-2 control-label ewLabel"><?php echo $main_Vendor->v_ID->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $main_Vendor->v_ID->CellAttributes() ?>>
<span id="el_main_Vendor_v_ID">
<span<?php echo $main_Vendor->v_ID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $main_Vendor->v_ID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="main_Vendor" data-field="x_v_ID" name="x_v_ID" id="x_v_ID" value="<?php echo ew_HtmlEncode($main_Vendor->v_ID->CurrentValue) ?>">
<?php echo $main_Vendor->v_ID->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
ew_CreateEditor("fmain_Vendoredit", "x_v_Note", 35, 3, <?php echo ($main_Vendor->v_Note->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php
	if (in_array("main_PartNum", explode(",", $main_Vendor->getCurrentDetailTable())) && $main_PartNum->DetailEdit) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $main_Vendor_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fmain_Vendoredit.Init();
</script>
<?php
$main_Vendor_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$main_Vendor_edit->Page_Terminate();
?>
