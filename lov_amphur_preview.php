...
$lov_amphur_preview = NULL; // Initialize page object first

class clov_amphur_preview extends clov_amphur {
	...
	// Page object name
	var $PageObjName = 'lov_amphur_preview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'lov_amphur');
	...
	function MasterKeyUrl() {
		$mastertblvar = @$_GET["t"];
		$url = "";
		if ($mastertblvar == "lov_province") {
			$url = "" . EW_TABLE_SHOW_MASTER . "=lov_province&fk_PROVINCE_ID=" . urlencode(strval($this->PROVINCE_ID->QueryStringValue)) . "";
		}
		return $url;
	}
		...
		if ($mastertblvar == "lov_province") {
			...
			if ($x !== FALSE) {
				...
				$val = $this->UnquoteValue($val, "DB");
 				$this->PROVINCE_ID->setQueryStringValue($val);
			}
		}
	}
...
// Create page object
if (!isset($lov_amphur_preview)) $lov_amphur_preview = new clov_amphur_preview();

// Page init
$lov_amphur_preview->Page_Init();

// Page main
$lov_amphur_preview->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$lov_amphur_preview->Page_Render();
?>
<?php $lov_amphur_preview->ShowPageHeader(); ?>
<?php if ($lov_amphur_preview->TotalRecs > 0) { ?>
<div class="ewGrid">
...
// Render list options
$lov_amphur_preview->RenderListOptions();

// Render list options (header, left)
$lov_amphur_preview->ListOptions->Render("header", "left");
?>
<?php if ($lov_amphur->AMPHUR_ID->Visible) { // AMPHUR_ID ?>
			<th><?php echo $lov_amphur->AMPHUR_ID->FldCaption() ?></th>
<?php } ?>
<?php if ($lov_amphur->AMPHUR_CODE->Visible) { // AMPHUR_CODE ?>
			<th><?php echo $lov_amphur->AMPHUR_CODE->FldCaption() ?></th>
<?php } ?>
<?php if ($lov_amphur->PROVINCE_ID->Visible) { // PROVINCE_ID ?>
			<th><?php echo $lov_amphur->PROVINCE_ID->FldCaption() ?></th>
<?php } ?>
<?php if ($lov_amphur->AMPHUR_NAME->Visible) { // AMPHUR_NAME ?>
			<th><?php echo $lov_amphur->AMPHUR_NAME->FldCaption() ?></th>
<?php } ?>
<?php if ($lov_amphur->AMPER_NAME_EN->Visible) { // AMPER_NAME_EN ?>
			<th><?php echo $lov_amphur->AMPER_NAME_EN->FldCaption() ?></th>
<?php } ?>
<?php

// Render list options (header, right)
$lov_amphur_preview->ListOptions->Render("header", "right");
?>
		</tr>
	</thead>
	<tbody><!-- Table body -->
<?php
$lov_amphur_preview->RecCount = 0;
$lov_amphur_preview->RowCnt = 0;
while ($lov_amphur_preview->Recordset && !$lov_amphur_preview->Recordset->EOF) {

	// Init row class and style
	$lov_amphur_preview->RecCount++;
	$lov_amphur_preview->RowCnt++;
	$lov_amphur_preview->CssStyle = "";
	$lov_amphur_preview->LoadListRowValues($lov_amphur_preview->Recordset);

	// Render row
	$lov_amphur_preview->RowType = EW_ROWTYPE_PREVIEW; // Preview record
	$lov_amphur_preview->RenderListRow();

	// Render list options
	$lov_amphur_preview->RenderListOptions();
?>
	<tr<?php echo $lov_amphur_preview->RowAttributes() ?>>
<?php

// Render list options (body, left)
$lov_amphur_preview->ListOptions->Render("body", "left", $lov_amphur_preview->RowCnt);
?>
<?php if ($lov_amphur->AMPHUR_ID->Visible) { // AMPHUR_ID ?>
		<!-- AMPHUR_ID -->
...
$lov_amphur_preview->ListOptions->Render("body", "right", $lov_amphur_preview->RowCnt);
?>
	</tr>
<?php
	$lov_amphur_preview->Recordset->MoveNext();
}
?>
	</tbody>
</table>
...
<?php if ($lov_amphur_preview->TotalRecs > 0) { ?>
<div class="ewDetailCount"><?php echo $lov_amphur_preview->TotalRecs ?>&nbsp;<?php echo $Language->Phrase("Record") ?></div>
<?php } else { ?>
<div class="ewDetailCount"><?php echo $Language->Phrase("NoRecord") ?></div>
<?php } ?>
<div class="ewPreviewOtherOptions">
<?php
	foreach ($lov_amphur_preview->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
...
<?php
$lov_amphur_preview->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php
if ($lov_amphur_preview->Recordset)
	$lov_amphur_preview->Recordset->Close();
...
?>
<?php
$lov_amphur_preview->Page_Terminate();
?>
