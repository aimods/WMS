...
$lov_ProductStatus_add = NULL; // Initialize page object first

class clov_ProductStatus_add extends clov_ProductStatus {
	...
	// Page object name
	var $PageObjName = 'lov_ProductStatus_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["ps_ID"] != "") {
				...
				$this->setKey("ps_ID", $this->ps_ID->CurrentValue); // Set up key
			} else {
				$this->setKey("ps_ID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "lov_productstatus_view.php")
	...
	function LoadDefaultValues() {
		$this->ps_Name->CurrentValue = NULL;
		$this->ps_Name->OldValue = $this->ps_Name->CurrentValue;
		$this->ps_inWarranty->CurrentValue = 0;
		$this->ps_DayofWarranty->CurrentValue = 90;
	}
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->ps_Name->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(ps_Name = '" . ew_AdjustSql($this->ps_Name->CurrentValue, $this->DBID) . "')";
		...
		// ps_Name
		$this->ps_Name->SetDbValueDef($rsnew, $this->ps_Name->CurrentValue, "", FALSE);

		// ps_inWarranty
		$this->ps_inWarranty->SetDbValueDef($rsnew, $this->ps_inWarranty->CurrentValue, 0, strval($this->ps_inWarranty->CurrentValue) == "");

		// ps_DayofWarranty
		$this->ps_DayofWarranty->SetDbValueDef($rsnew, $this->ps_DayofWarranty->CurrentValue, 0, strval($this->ps_DayofWarranty->CurrentValue) == "");
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->ps_ID->setDbValue($conn->Insert_ID());
				$rsnew['ps_ID'] = $this->ps_ID->DbValue;
			}
		} else {
...
// Create page object
if (!isset($lov_ProductStatus_add)) $lov_ProductStatus_add = new clov_ProductStatus_add();

// Page init
$lov_ProductStatus_add->Page_Init();

// Page main
$lov_ProductStatus_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$lov_ProductStatus_add->Page_Render();
?>
...
var CurrentForm = flov_ProductStatusadd = new ew_Form("flov_ProductStatusadd", "add");

// Validate form
flov_ProductStatusadd.Validate = function() {
...
flov_ProductStatusadd.Form_CustomValidate = 
...
flov_ProductStatusadd.ValidateRequired = true;
<?php } else { ?>
flov_ProductStatusadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
flov_ProductStatusadd.Lists["x_ps_inWarranty"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
flov_ProductStatusadd.Lists["x_ps_inWarranty"].Options = <?php echo json_encode($lov_ProductStatus->ps_inWarranty->Options()) ?>;
...
<?php $lov_ProductStatus_add->ShowPageHeader(); ?>
<?php
$lov_ProductStatus_add->ShowMessage();
?>
<form name="flov_ProductStatusadd" id="flov_ProductStatusadd" class="<?php echo $lov_ProductStatus_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($lov_ProductStatus_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $lov_ProductStatus_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $lov_ProductStatus_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
flov_ProductStatusadd.Init();
</script>
<?php
$lov_ProductStatus_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$lov_ProductStatus_add->Page_Terminate();
?>
