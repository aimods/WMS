...
$lov_WHType_add = NULL; // Initialize page object first

class clov_WHType_add extends clov_WHType {
	...
	// Page object name
	var $PageObjName = 'lov_WHType_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["wh_ID"] != "") {
				...
				$this->setKey("wh_ID", $this->wh_ID->CurrentValue); // Set up key
			} else {
				$this->setKey("wh_ID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "lov_whtype_view.php")
	...
	function LoadDefaultValues() {
		$this->wh_Type->CurrentValue = NULL;
		$this->wh_Type->OldValue = $this->wh_Type->CurrentValue;
		$this->wh_isVirtual->CurrentValue = 0;
		$this->wh_TypeNote->CurrentValue = NULL;
		$this->wh_TypeNote->OldValue = $this->wh_TypeNote->CurrentValue;
	}
	...
	function RenderRow() {
		...
		$this->wh_TypeNote->ViewCustomAttributes = "";
			...
			$this->wh_Type->EditValue = ew_HtmlEncode($this->wh_Type->CurrentValue);
			$this->wh_Type->PlaceHolder = ew_RemoveHtml($this->wh_Type->FldCaption());
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->wh_Type->FldIsDetailKey && !is_null($this->wh_Type->FormValue) && $this->wh_Type->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->wh_Type->FldCaption(), $this->wh_Type->ReqErrMsg));
		}
	...
	function AddRow($rsold = NULL) {
		...
		// wh_Type
		$this->wh_Type->SetDbValueDef($rsnew, $this->wh_Type->CurrentValue, "", FALSE);

		// wh_isVirtual
		$this->wh_isVirtual->SetDbValueDef($rsnew, $this->wh_isVirtual->CurrentValue, 0, strval($this->wh_isVirtual->CurrentValue) == "");

		// wh_TypeNote
		$this->wh_TypeNote->SetDbValueDef($rsnew, $this->wh_TypeNote->CurrentValue, NULL, FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->wh_ID->setDbValue($conn->Insert_ID());
				$rsnew['wh_ID'] = $this->wh_ID->DbValue;
			}
		} else {
...
// Create page object
if (!isset($lov_WHType_add)) $lov_WHType_add = new clov_WHType_add();

// Page init
$lov_WHType_add->Page_Init();

// Page main
$lov_WHType_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$lov_WHType_add->Page_Render();
?>
...
var CurrentForm = flov_WHTypeadd = new ew_Form("flov_WHTypeadd", "add");

// Validate form
flov_WHTypeadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			elm = this.GetElements("x" + infix + "_wh_Type");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $lov_WHType->wh_Type->FldCaption(), $lov_WHType->wh_Type->ReqErrMsg)) ?>");
...
flov_WHTypeadd.Form_CustomValidate = 
...
flov_WHTypeadd.ValidateRequired = true;
<?php } else { ?>
flov_WHTypeadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
flov_WHTypeadd.Lists["x_wh_isVirtual"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
flov_WHTypeadd.Lists["x_wh_isVirtual"].Options = <?php echo json_encode($lov_WHType->wh_isVirtual->Options()) ?>;
...
<?php $lov_WHType_add->ShowPageHeader(); ?>
<?php
$lov_WHType_add->ShowMessage();
?>
<form name="flov_WHTypeadd" id="flov_WHTypeadd" class="<?php echo $lov_WHType_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($lov_WHType_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $lov_WHType_add->Token ?>">
<?php } ?>
...
<?php if ($lov_WHType->wh_Type->Visible) { // wh_Type ?>
	<div id="r_wh_Type" class="form-group">
		<label id="elh_lov_WHType_wh_Type" for="x_wh_Type" class="col-sm-2 control-label ewLabel"><?php echo $lov_WHType->wh_Type->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $lov_WHType->wh_Type->CellAttributes() ?>>
<span id="el_lov_WHType_wh_Type">
<input type="text" data-table="lov_WHType" data-field="x_wh_Type" name="x_wh_Type" id="x_wh_Type" size="30" maxlength="30" placeholder="<?php echo ew_HtmlEncode($lov_WHType->wh_Type->getPlaceHolder()) ?>" value="<?php echo $lov_WHType->wh_Type->EditValue ?>"<?php echo $lov_WHType->wh_Type->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $lov_WHType_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
