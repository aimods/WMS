...
$lov_amphur_edit = NULL; // Initialize page object first

class clov_amphur_edit extends clov_amphur {
	...
	// Page object name
	var $PageObjName = 'lov_amphur_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->AMPHUR_ID->CurrentValue == "")
			$this->Page_Terminate("lov_amphur_list.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->AMPHUR_ID->FldIsDetailKey)
			$this->AMPHUR_ID->setFormValue($objForm->GetValue("x_AMPHUR_ID"));
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// AMPHUR_NAME
			$this->AMPHUR_NAME->SetDbValueDef($rsnew, $this->AMPHUR_NAME->CurrentValue, "", $this->AMPHUR_NAME->ReadOnly);

			// AMPER_NAME_EN
			$this->AMPER_NAME_EN->SetDbValueDef($rsnew, $this->AMPER_NAME_EN->CurrentValue, NULL, $this->AMPER_NAME_EN->ReadOnly);
...
// Create page object
if (!isset($lov_amphur_edit)) $lov_amphur_edit = new clov_amphur_edit();

// Page init
$lov_amphur_edit->Page_Init();

// Page main
$lov_amphur_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$lov_amphur_edit->Page_Render();
?>
...
var CurrentForm = flov_amphuredit = new ew_Form("flov_amphuredit", "edit");

// Validate form
flov_amphuredit.Validate = function() {
...
flov_amphuredit.Form_CustomValidate = 
...
flov_amphuredit.ValidateRequired = true;
<?php } else { ?>
flov_amphuredit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
flov_amphuredit.Lists["x_PROVINCE_ID"] = {"LinkField":"x_PROVINCE_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_PROVINCE_NAME","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $lov_amphur_edit->ShowPageHeader(); ?>
<?php
$lov_amphur_edit->ShowMessage();
?>
<form name="flov_amphuredit" id="flov_amphuredit" class="<?php echo $lov_amphur_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($lov_amphur_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $lov_amphur_edit->Token ?>">
<?php } ?>
...
<?php if ($lov_amphur->AMPHUR_ID->Visible) { // AMPHUR_ID ?>
	<div id="r_AMPHUR_ID" class="form-group">
		<label id="elh_lov_amphur_AMPHUR_ID" class="col-sm-2 control-label ewLabel"><?php echo $lov_amphur->AMPHUR_ID->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $lov_amphur->AMPHUR_ID->CellAttributes() ?>>
<span id="el_lov_amphur_AMPHUR_ID">
...
<input type="hidden" data-table="lov_amphur" data-field="x_AMPHUR_ID" name="x_AMPHUR_ID" id="x_AMPHUR_ID" value="<?php echo ew_HtmlEncode($lov_amphur->AMPHUR_ID->CurrentValue) ?>">
<?php echo $lov_amphur->AMPHUR_ID->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($lov_amphur->AMPHUR_CODE->Visible) { // AMPHUR_CODE ?>
	<div id="r_AMPHUR_CODE" class="form-group">
		<label id="elh_lov_amphur_AMPHUR_CODE" for="x_AMPHUR_CODE" class="col-sm-2 control-label ewLabel"><?php echo $lov_amphur->AMPHUR_CODE->FldCaption() ?></label>
...
<input type="hidden" data-table="lov_amphur" data-field="x_AMPHUR_CODE" name="x_AMPHUR_CODE" id="x_AMPHUR_CODE" value="<?php echo ew_HtmlEncode($lov_amphur->AMPHUR_CODE->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $lov_amphur_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
flov_amphuredit.Init();
</script>
<?php
$lov_amphur_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$lov_amphur_edit->Page_Terminate();
?>
