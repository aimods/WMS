...
class cforgotpwd extends cmain_User {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["main_User"])) $GLOBALS["main_User"] = &$this;
		...
		if ($this->Action <> "") {
			...
			if ($RsUser = $UserTableConn->Execute($sSql)) {
				if (!$RsUser->EOF) {
					...
					if ($bValidEmail) {
						$sUserName = $rsold['u_LoginName'];
						$sPassword = $rsold['u_Passwd'];
					}
				} else {
					$bValidEmail = FALSE;
...
<?php } ?>
	<div class="form-group">
		...
		<div class="col-sm-10"><input type="text" name="email" id="email" class="form-control ewControl" value="<?php ew_HtmlEncode($forgotpwd->Email) ?>" size="30" maxlength="120" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("UserEmail")) ?>"></div>
	</div>
	<div class="form-group">
