...
$StockCard_grid = NULL; // Initialize page object first

class cStockCard_grid extends cStockCard {
	...
	// Page object name
	var $PageObjName = 'StockCard_grid';

	// Grid form hidden field names
	var $FormName = 'fStockCardgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["StockCard"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_PartNo") && $objForm->HasValue("o_PartNo") && $this->PartNo->CurrentValue <> $this->PartNo->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ProductName") && $objForm->HasValue("o_ProductName") && $this->ProductName->CurrentValue <> $this->ProductName->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Status") && $objForm->HasValue("o_Status") && $this->Status->CurrentValue <> $this->Status->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Items") && $objForm->HasValue("o_Items") && $this->Items->CurrentValue <> $this->Items->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		if (!$this->PartNo->FldIsDetailKey) {
			$this->PartNo->setFormValue($objForm->GetValue("x_PartNo"));
		}
		$this->PartNo->setOldValue($objForm->GetValue("o_PartNo"));
		if (!$this->ProductName->FldIsDetailKey) {
			$this->ProductName->setFormValue($objForm->GetValue("x_ProductName"));
		}
		$this->ProductName->setOldValue($objForm->GetValue("o_ProductName"));
		...
		$this->Items->setOldValue($objForm->GetValue("o_Items"));
	}
	...
	function RenderRow() {
		...
		$this->Items->ViewCustomAttributes = "";
			...
			$this->PartNo->EditValue = ew_HtmlEncode($this->PartNo->CurrentValue);
			...
			$this->Items->EditValue = ew_HtmlEncode($this->Items->CurrentValue);
			...
			$this->PartNo->LinkCustomAttributes = "";
			if (!ew_Empty($this->LinkedID->CurrentValue)) {
				$this->PartNo->HrefValue = "stockcard_list.php?showmaster=main_PartNum&fk_pn_ID=" . ((!empty($this->LinkedID->EditValue)) ? ew_RemoveHtml($this->LinkedID->EditValue) : $this->LinkedID->CurrentValue); // Add prefix/suffix
				$this->PartNo->LinkAttrs["target"] = ""; // Add target
				if ($this->Export <> "") $this->PartNo->HrefValue = ew_ConvertFullUrl($this->PartNo->HrefValue);
			} else {
				$this->PartNo->HrefValue = "";
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->PartNo->FldIsDetailKey && !is_null($this->PartNo->FormValue) && $this->PartNo->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->PartNo->FldCaption(), $this->PartNo->ReqErrMsg));
		}
		...
		if (!ew_CheckInteger($this->Items->FormValue)) {
			ew_AddMessage($gsFormError, $this->Items->FldErrMsg());
		}
	...
	function AddRow($rsold = NULL) {
		...
		// PartNo
		$this->PartNo->SetDbValueDef($rsnew, $this->PartNo->CurrentValue, NULL, FALSE);
		...
		if ($this->LinkedID->getSessionValue() <> "") {
			$rsnew['LinkedID'] = $this->LinkedID->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "main_PartNum") {
			$this->LinkedID->Visible = FALSE;
			if ($GLOBALS["main_PartNum"]->EventCancelled) $this->EventCancelled = TRUE;
		}
